...
function patchSingleFrame(frame, page, cfg, cursor, raw, rawKb, originals, stealth) {
    ...
    const moveToFrameSelector = async (selector, options, inputBias = false) => {
        const callCfg = mergeConfig(cfg, options?.human_config ?? options);
        if (callCfg.idle_between_actions) {
            await humanIdle(raw, cursor.x, cursor.y, callCfg);
        }
    ...
    const frameClick = async (selector, options) => {
        ...
        if (!moved)
            return origFrameClick(selector, options);
    ...
    const frameHover = async (selector, options) => {
        ...
        if (!moved)
            return origFrameHover(selector, options);
    };
    frame.click = frameClick;
    frame.dblclick = async (selector, options) => {
        const moved = await moveToFrameSelector(selector, options);
        if (!moved)
            return origFrameDblclick(selector, options);
        await raw.down({ clickCount: 2 });
        await sleep(rand(30, 60));
        await raw.up({ clickCount: 2 });
    };
